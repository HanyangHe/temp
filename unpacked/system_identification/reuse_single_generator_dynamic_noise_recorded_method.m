function [methodResult,replayInfo] = reuse_single_generator_dynamic_noise_recorded_method( ...
    sourceRoot,roundIndex,noiseLevel,methodField,methodLabel,strictMode,task,compactMode)
%REUSE_SINGLE_GENERATOR_DYNAMIC_NOISE_RECORDED_METHOD Replay one saved noise-case method.
%
% The derivative-noise experiment is keyed by (round, rho), not by
% (round, Ntrain). Dedicated checkpoints are stored as
%   round_XX/rho_YYY_permil/method_results/<methodField>.mat
% by save_single_generator_dynamic_noise_run. Each file contains variable
...
% sample-efficiency replay path.
    ...
    if exist(methodPath,'file') == 2
        ...
        if isfield(loaded,'result') && isstruct(loaded.result) && ~isempty(loaded.result)
            ...
            warning('Ignoring malformed noise method checkpoint: %s',methodPath);
        end
    end
    ...
    if isempty(methodResult)
        aggregatePath = fullfile(sourceRoot,'summary', ...
            'single_generator_dynamic_noise_robustness_results.mat');
    ...
    if isempty(methodResult)
        message = sprintf(['Saved %s noise-case result was not found for %s/%s under:\n%s\n', ...
            ...
            'to a valid noise-robustness output tree.'], ...
    ...
    % not require record_single_generator_dynamic_method_report.
    if isStage0SR

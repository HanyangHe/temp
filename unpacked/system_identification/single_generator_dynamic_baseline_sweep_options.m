function sweep = single_generator_dynamic_baseline_sweep_options(projectRoot, stage0PythonExe)
%SINGLE_GENERATOR_DYNAMIC_BASELINE_SWEEP_OPTIONS Case-local baseline sweeps.
%
% Keep the low-dimensional generator benchmark policy in one editable file.
...
% the configured relative tolerance for the configured patience.
    ...
    mlp.workRoot = fullfile(projectRoot,'tmp','mlp_single_generator_runs');
    ...
    kan.workRoot = fullfile(projectRoot,'tmp','kan_single_generator_runs');
    kan.width = 8;
    kan.depthList = 2:6;
    ...
    eql.workRoot = fullfile(projectRoot,'tmp','eql_single_generator_runs');

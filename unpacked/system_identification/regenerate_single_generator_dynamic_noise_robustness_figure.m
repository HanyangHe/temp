function info = regenerate_single_generator_dynamic_noise_robustness_figure(resultsFile,exportDir,varargin)
%REGENERATE_SINGLE_GENERATOR_DYNAMIC_NOISE_ROBUSTNESS_FIGURE Recompute RMSE summary and replot saved results.
%
...
% figure/CSV/figure-data artifacts.
    ...
    addParameter(p,'YLimits',[1e-5,1],@(x)isnumeric(x)&&numel(x)==2&&all(isfinite(x))&&x(1)>0&&x(2)>x(1));
    parse(p,varargin{:});
    ...
    [fig,figureData,summaryTable]=plot_single_generator_dynamic_noise_robustness( ...
        rows,'Visible',logical(p.Results.Visible),'YLimits',p.Results.YLimits);
    figPath=fullfile(exportDir,'generator_noise_robustness.fig');
    pdfPath=fullfile(exportDir,'generator_noise_robustness.pdf');
    dataPath=fullfile(exportDir,'generator_noise_robustness_figure_data.mat');
    csvPath=fullfile(exportDir,'generator_noise_robustness_mean_std.csv');
    savefig(fig,figPath);

function result = record_soft_saturated_lorenz96_method_report(result, methodField, task)
%RECORD_SOFT_SATURATED_LORENZ96_METHOD_REPORT Store a replayable text report.
%
...
function reportText = render_report_local(result,methodField,task)
    ...
    switch methodField
        case 'mlp'
            reportText = evalc('print_soft_saturated_lorenz96_mlp_result(result);');
        case 'eql'
            reportText = evalc('print_soft_saturated_lorenz96_eql_result(result);');
        case 'kan'
            reportText = evalc('print_soft_saturated_lorenz96_kan_result(result);');
        case {'sindy','neural_sindy'}

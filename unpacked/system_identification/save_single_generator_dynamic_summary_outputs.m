function outputInfo = save_single_generator_dynamic_summary_outputs( ...
    ...
    runMetadata)
%SAVE_SINGLE_GENERATOR_DYNAMIC_SUMMARY_OUTPUTS Persist a slim aggregate summary.
%
...
% The allResults input is retained only for backward-compatible call syntax.
    ...
    outputInfo.legacyResultsMatPath = fullfile(summaryDir,'single_generator_dynamic_results.mat');
    ...
    fprintf('Saved slim aggregate SingleGeneratorDynamic summary to:\n%s\n', ...
...
function write_metadata_text_local(meta,path)
    ...
    fprintf(fid,'SingleGeneratorDynamic SMIB-AVR run metadata\n');

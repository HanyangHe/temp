function [predictFcn, info] = make_soft_saturated_lorenz96_predictor(methodName, methodResult)
%MAKE_SOFT_SATURATED_LORENZ96_PREDICTOR Build one fixed rollout predictor.
%
...
% routine instead of terminating the complete rollout script.
    ...
    switch methodFamily
        case 'phdn'
            ...
            compileTimer = tic;
            try
                compiled = compile_soft_saturated_lorenz96_phdn_predictor(methodResult);
                predictFcn = @(X) predict_compiled_soft_saturated_lorenz96_phdn(compiled, X);
                info.available = true;
                info.compiled = true;
                ...
                info.reason = sprintf(['Compiled PhDN inference was unavailable; ', ...
                    ...
                    ME.message);
                warning('SoftSaturatedLorenz96:CompiledPhDNFallback', '%s', info.reason);
            end
        ...
        case {'stage0-sr', 'stage0sr'}
            ...
            predictFcn = @(X) predict_soft_saturated_lorenz96_stage0_sr( ...
                methodResult, X);
            info.available = true;
        ...
        case 'sindy'
            ...
            compileTimer = tic;
            try
                compiled = compile_soft_saturated_lorenz96_sindy_predictor(methodResult);
                predictFcn = @(X) predict_compiled_soft_saturated_lorenz96_sindy(compiled, X);
                info.available = true;
                info.compiled = true;
                info.compileTime = toc(compileTimer);
                info.activeTermCount = compiled.activeTermCount;
                ...
                warning('SoftSaturatedLorenz96:CompiledSINDyFallback', '%s', info.reason);
            end

        case {'kan','kan-pruned'}
            if isfield(methodResult,'portableModel') && isstruct(methodResult.portableModel) && ...
                    ~isempty(fieldnames(methodResult.portableModel))
                predictFcn=@(X) predict_soft_saturated_lorenz96_kan_baseline(methodResult,X);
        ...
        case {'eql','eql-div'}
            if isfield(methodResult,'portableModel') && isstruct(methodResult.portableModel) && ...
                    ~isempty(fieldnames(methodResult.portableModel))
                predictFcn=@(X) predict_soft_saturated_lorenz96_eql_baseline(methodResult,X);
                info.available=true; info.compiled=true;
        ...
        case 'mlp'
            ...
            elseif isfield(methodResult, 'modelParameters') && ...
                ...
                predictFcn = @(X) predict_soft_saturated_lorenz96_mlp_baseline( ...
                    methodResult, X);
                info.available = true;

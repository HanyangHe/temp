function [figs, savedPdfPath] = ...
    plot_soft_saturated_lorenz96_trajectory_all_samples(task, rows, varargin)
%PLOT_SOFT_SATURATED_LORENZ96_TRAJECTORY_ALL_SAMPLES
...
%
% Usage:
%   [figs,savedPdfPath] = plot_soft_saturated_lorenz96_trajectory_all_samples( ...
...
%       'SavePdf',true, ...
%       'SavePdfPath',fullfile(pwd,'soft_saturated_lorenz96_rollout_selected.pdf'));
%
% Name-value options:
...
% automates figure generation for multiple N values.
    ...
    for k = 1:numel(sampleList)
        ...
        fig = plot_soft_saturated_lorenz96_trajectory(task,rows,currentN);

        if isgraphics(fig)
            set(fig,'Visible',char(opt.Visible));
            set(fig,'Name',sprintf('SoftSaturatedLorenz96 rollout N=%d',currentN));
            figs(k) = fig;
        else
            figs(k) = gobjects(1);
        end
    ...
    if logical(opt.SavePdf)
        ...
        if strlength(string(opt.SavePdfPath)) == 0
            savedPdfPath = fullfile(pwd, ...
                sprintf('soft_saturated_lorenz96_rollout_N_%05d.pdf',saveSelectedNTrain));
        else

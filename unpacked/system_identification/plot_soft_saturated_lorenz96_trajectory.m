function [fig, plotData] = plot_soft_saturated_lorenz96_trajectory(task, rows, selectedNTrain)
%PLOT_SOFT_SATURATED_LORENZ96_TRAJECTORY Mean rollout over every IC and round.
...
% of the true trajectories over the same rollout IC set.
    ...
    fig = figure('Name',sprintf('SoftSaturatedLorenz96 rollout mean over all ICs N=%d',selectedNTrain), ...
        'Color','w','Visible','on');
    nColumns = 2;
    nRows = ceil(task.nx/nColumns);
    tiledlayout(nRows,nColumns,'TileSpacing','compact','Padding','compact');
    labels = arrayfun(@(k) sprintf('x_{%d}',k),1:task.nx,'UniformOutput',false);
    ...
    sgtitle(sprintf(['Soft-saturated Lorenz--96 alternating joint-OOD rollout ', ...

function X = sample_single_generator_dynamic_split(nSamples, domain, plan)
%SAMPLE_SINGLE_GENERATOR_DYNAMIC_SPLIT Core--shell scrambled-Sobol data design.
%
% The SingleGeneratorDynamic sample-efficiency experiment uses four distinct,
...
% shared framework still returns the intended exact split.
    ...
    if ~strcmpi(strtrim(char(samplingMethod)),'scrambled_sobol')
        error('SingleGeneratorDynamic requires samplingMethod=''scrambled_sobol''; received %s.', ...
...
% Uniform scrambled-Sobol sampling in the centered rectangular ID core.
    ...
    coreDomain.source = 'SingleGeneratorDynamic_ID_training_core';
...
% Generate a reproducible scrambled-Sobol pool and remove exact overlaps.
    ...
    if any(nIntervals ~= 1)
        error(['The SingleGeneratorDynamic Sobol sampler currently requires ', ...

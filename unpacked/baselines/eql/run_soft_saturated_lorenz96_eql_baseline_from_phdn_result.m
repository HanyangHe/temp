function resultEql=run_soft_saturated_lorenz96_eql_baseline_from_phdn_result(resultPhdn,eqlOpts)
%RUN_SOFT_SATURATED_LORENZ96_EQL_BASELINE_FROM_PHDN_RESULT Case-local EQL sweep.
%
...
% more likely without silently relabelling a smaller-N model as an N-sample one.
    ...
    freshResult=train_soft_saturated_lorenz96_eql_sweep_baseline( ...
    ...
    if getfield_default_local(eqlOpts,'verbose',true)
        print_soft_saturated_lorenz96_eql_result(resultEql);
    end
end
...
function [info,opts]=prepare_previous_reference_local(opts,d,XOod,YOod,currentN)
    ...
    try
        ...
        info.YTrainPred=predict_soft_saturated_lorenz96_eql_baseline(prev,d.Xtr);
        info.YValPred=predict_soft_saturated_lorenz96_eql_baseline(prev,d.Xval);
        info.YTestPred=predict_soft_saturated_lorenz96_eql_baseline(prev,d.Xte);
        if isempty(XOod)
            info.YOodPred=zeros(0,size(d.Ytr,2));
        else
            info.YOodPred=predict_soft_saturated_lorenz96_eql_baseline(prev,XOod);
        end

function result = train_soft_saturated_lorenz96_eql_sweep_baseline(XTrain,YTrain,XVal,YVal,XTest,YTest,XOod,YOod,eqlOpts,varargin)
%TRAIN_SOFT_SATURATED_LORENZ96_EQL_SWEEP_BASELINE Run the bundled official EQL-Div Theano sweep.
%
...
% its penalty-epoch sampling and loss.
    ...
    adapterPath=fullfile(eqlDir,'eql_official_adapter_soft_saturated_lorenz96.py');
    ...
    if ~isempty(fieldnames(result.portableModel))
        portableTest=predict_soft_saturated_lorenz96_eql_baseline(result,XTest);

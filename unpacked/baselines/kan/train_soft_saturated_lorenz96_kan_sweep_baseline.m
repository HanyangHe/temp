function result=train_soft_saturated_lorenz96_kan_sweep_baseline(XTrain,YTrain,XVal,YVal,XTest,YTest,XOod,YOod,kanOpts)
%TRAIN_SOFT_SATURATED_LORENZ96_KAN_SWEEP_BASELINE Accuracy-first SI pyKAN sweep.
...
% still completes zero-lambda recovery plus the remaining grid schedule.
    ...
    adapterPath=fullfile(kanDir,'kan_paper_pruned_adapter_soft_saturated_lorenz96.py');
    ...
    if ~isempty(fieldnames(result.portableModel))
        portableTest=predict_soft_saturated_lorenz96_kan_baseline(result,XTest);

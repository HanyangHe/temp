...
def main() -> int:
    ...
    configured_candidate_count=len(depths)*len(acts)
    candidates=[]; selected=None; selected_model=None
    depth_stages=[]; best_depth_val_mse=float('inf'); best_depth=None
    bad_depth_count=0; last_attempted_depth=None; depth_stop_reason=''
    for depth in depths:
      last_attempted_depth=depth
      depth_candidate_start=len(candidates)
      ...
      depth_candidates=[c for c in candidates[depth_candidate_start:] if c.get('status')=='ok' and np.isfinite(c.get('val_metrics',{}).get('mse',float('nan')))]
      if depth_candidates:
        depth_best=min(depth_candidates,key=lambda c:(c['val_metrics']['mse'],c['parameter_count'],acts.index(c['activation'])))
        current_depth_val=float(depth_best['val_metrics']['mse'])
        previous_best=best_depth_val_mse
        is_new_best=current_depth_val<best_depth_val_mse
        meaningful_rise=(np.isfinite(best_depth_val_mse) and current_depth_val>best_depth_val_mse*(1.0+depth_early_stop_relative_tolerance))
        if is_new_best:
          best_depth_val_mse=current_depth_val; best_depth=depth; bad_depth_count=0
        elif meaningful_rise:
          bad_depth_count+=1
        else:
          bad_depth_count=0
        should_stop=depth_early_stop and meaningful_rise and bad_depth_count>=depth_early_stop_patience
        stage_reason=''
        if should_stop:
          stage_reason=f'validation_mse_increased_at_depth_{depth}; restore_best_depth_{best_depth}'
          depth_stop_reason=stage_reason
        depth_stages.append(dict(depth=depth,status='early_stop_trigger' if should_stop else 'ok',
          best_activation=depth_best['activation'],best_val_mse=current_depth_val,
          previous_global_best_val_mse=previous_best,accepted=is_new_best,
          bad_depth_count=bad_depth_count,stop_reason=stage_reason))
        print(f"[MLP depth sweep] depth={depth} best_act={depth_best['activation']} best_val_mse={current_depth_val:.6e} accepted={int(is_new_best)} stop={int(should_stop)}",flush=True)
        if should_stop:
          break
      else:
        depth_stages.append(dict(depth=depth,status='all_candidates_failed',best_activation='',
          best_val_mse=float('nan'),previous_global_best_val_mse=best_depth_val_mse,
          accepted=False,bad_depth_count=bad_depth_count,stop_reason=''))
    if not depth_stop_reason:
      depth_stop_reason='depth_schedule_completed' if depth_early_stop else 'disabled'
    ...
    model_parameters=dict(activation=sc['activation'],
      weights=[layer.weight.detach().cpu().numpy().tolist() for layer in selected_model.linears],
      biases=[layer.bias.detach().cpu().numpy().tolist() for layer in selected_model.linears])
    result=dict(method='KAN-paper MLP sweep',protocol='kan_feynman_mlp_sweep',selection_metric='validation_mse',
      ...
      ood_metrics=metrics(Yo,yp_o),candidate_count=len(candidates),configured_candidate_count=configured_candidate_count,
      candidates=candidates,depth_early_stop=depth_early_stop,depth_stop_reason=depth_stop_reason,
      last_attempted_depth=last_attempted_depth,best_depth=best_depth,depth_stages=depth_stages,
      normalization=dict(x_mean=xmu,x_std=xsd,y_mean=ymu,y_std=ysd),model_parameters=model_parameters)

...
% already-completed Stage-0 search; it has no separate switch, options, or rerun.
...
for iRound = 1:NumRounds
    ...
    runAnyBaseline = RunMLPBaseline || RunEQLBaseline || RunKANBaseline || RunSINDyBaseline;
    result = [];
    baselineDataResult = [];
    ...
    if RunPhDNMainModel
        ...
        if isfield(result, 'ablations') && isstruct(result.ablations) && ...
                isfield(result.ablations, 'stage0SR')
    ...
    elseif runAnyBaseline
        ...
        fprintf('No PhDN or baseline method is enabled; skip this case.\n');
        continue;
    end
    ...
    if RunMLPBaseline
        ...
        mlpOpts.protocol = MLPProtocol;
        mlpOpts.pythonExe = MLPPythonExe;
        mlpOpts.pykanRoot = PyKANRoot;
        mlpOpts.workRoot = MLPWorkRoot;
        mlpOpts.width = MLPWidth;
        mlpOpts.depthList = MLPDepthList;
        mlpOpts.activationList = MLPActivationList;
        mlpOpts.optimizer = MLPOptimizer;
        mlpOpts.steps = MLPTrainSteps;
        mlpOpts.learningRate = MLPLearningRate;
        mlpOpts.verbose = MLPVerbose;
        mlpOpts.displaySweepTable = MLPDisplaySweepTable;
        resultMlp = run_mlp_baseline_from_phdn_result(baselineDataResult, mlpOpts);
        resultPack.mlp = resultMlp;
        if RunPhDNMainModel
            result.baselines.mlp = resultMlp;
        end
    end

    if RunEQLBaseline
        eqlOpts = make_default_eql_options_for_demo(roundEqlSeed);
        eqlOpts.pythonExe = EQLPythonExe;
        eqlOpts.officialRoot = EQLOfficialRoot;
        eqlOpts.workRoot = EQLWorkRoot;
        eqlOpts.depthList = EQLDepthList;
        eqlOpts.lambdaList = EQLLambdaList;
        eqlOpts.unitsPerUnaryType = EQLUnitsPerUnaryType;
        eqlOpts.multiplicationUnits = EQLUnitsPerUnaryType;
        eqlOpts.stepsPerHiddenLayer = EQLStepsPerHiddenLayer;
        eqlOpts.batchSize = EQLBatchSize;
        eqlOpts.learningRate = EQLLearningRate;
        eqlOpts.gradient = EQLGradient;
        eqlOpts.lambdaL2 = EQLLambdaL2;
        eqlOpts.penaltyEvery = EQLPenaltyEvery;
        eqlOpts.validateEvery = EQLValidateEvery;
        eqlOpts.candidateWorkers = EQLCandidateWorkers;
        eqlOpts.normalizeInputs = EQLNormalizeInputs;
        eqlOpts.normalizeOutputs = EQLNormalizeOutputs;
        eqlOpts.theanoFlags = EQLTheanoFlags;
        eqlOpts.verbose = EQLVerbose;
        eqlOpts.displaySweepTable = EQLDisplaySweepTable;
        resultEql = run_eql_baseline_from_phdn_result(baselineDataResult, eqlOpts);
        resultPack.eql = resultEql;
        if RunPhDNMainModel
            result.baselines.eql = resultEql;
        end
    end

    if RunKANBaseline
        kanOpts = make_default_kan_options_for_demo(roundKanSeed);
        kanOpts.pythonExe = KANPythonExe;
        kanOpts.pykanRoot = PyKANRoot;
        kanOpts.workRoot = KANWorkRoot;
        kanOpts.width = KANWidth;
        kanOpts.depthList = KANDepthList;
        kanOpts.gridList = KANGridList;
        kanOpts.splineOrder = KANSplineOrder;
        kanOpts.sparsificationLambdaList = KANSparsificationLambdaList;
        kanOpts.stepsPerGrid = KANStepsPerGrid;
        kanOpts.optimizer = KANOptimizer;
        kanOpts.learningRate = KANLearningRate;
        kanOpts.pruneNodeThreshold = KANPruneNodeThreshold;
        kanOpts.pruneEdgeThreshold = KANPruneEdgeThreshold;
        kanOpts.verbose = KANVerbose;
        kanOpts.displaySweepTable = KANDisplaySweepTable;
        resultKan = run_kan_baseline_from_phdn_result(baselineDataResult, kanOpts);
        resultPack.kan = resultKan;
        if RunPhDNMainModel
            result.baselines.kan = resultKan;
        end
    end
    ...
    if RunSINDyBaseline
        ...
        resultSindy = run_sindy_baseline_from_phdn_result(baselineDataResult, task, sindyOpts, opts);
        resultPack.sindy = resultSindy;
        if RunPhDNMainModel
            result.baselines.sindy = resultSindy;
        end
    end
